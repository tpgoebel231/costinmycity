...
export function CitiesDirectory({ cities }: { cities: City[] }) {
  ...
  return (
    <div className="mt-6">
      <div className="sticky top-16 z-40 border border-line bg-paper p-4 shadow-sm sm:top-[4.5rem]">
        ...
        />
        {groups.length > 0 ? (
          <nav aria-label="Jump to state" className="mt-3 flex flex-wrap gap-x-3 gap-y-1 text-sm">
            {groups.map((g) => (
              <a
                key={g.stateAbbr}
      ...
      {groups.map((g) => (
        <section key={g.stateAbbr} id={stateAnchorId(g.stateAbbr)} className="mt-8 scroll-mt-36">
